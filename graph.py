...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return "{0}->{1} ({2}, {3})".format(str(self.src), str(self.dest), str(self.total_distance), str(self.outdoor_distance))
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if self.has_node(edge.get_source()) and self.has_node(edge.get_destination()):
            self.edges.setdefault(edge.get_source().__hash__(), []).append(edge)
        else:
...
def test_edges_nodes():
    node1 = Node('x')
    node2 = Node('y')
    edge1 = WeightedEdge(node1, node2, 9, 7)
    graph = Digraph()
    ...
    node3 = Node('z')
    node4 = Node('w')
    edge2 = WeightedEdge(node3, node4, 14, 8)
    graph.add_node(node3)
    graph.add_node(node4)
    graph.add_edge(edge2)    
    # graph.add_edge(WeightedEdge(Node('a'), Node('b'), 7, 4))    
